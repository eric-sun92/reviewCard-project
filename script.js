...
const averageReview = Object.entries(REVIEWS).reduce((sum, [value, quantity]) => {
...
function update(time){
    if(startTime != null){
        ...
        const countElems = document.querySelectorAll('.review-count[data-end-value')

        countElems.forEach(elem => {

            elem.textContent = Math.round(getNewValue(
                elem.dataset.endValue,
                timeElapsed
                ))
        })

        const barElems = document.querySelectorAll('.review-bar[data-end-value')

        barElems.forEach(elem => {
            elem.style.setProperty("--width",  `${getNewValue(
                elem.dataset.endValue,
                timeElapsed
            )}%`)
        })
